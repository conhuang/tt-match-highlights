...
COMMAND_ID=$(aws ssm send-command \
  ...
  --parameters "commands=[
    ...
    \"docker run -d --name tt_app --restart always -p 80:8000 -e STORAGE_TYPE=s3 -e S3_BUCKET_NAME=tt-video-editor-storage-test -e DATABASE_TYPE=dynamodb -e DYNAMODB_TABLE_NAME=tt_video_editor_matches_dev -e AWS_REGION=$AWS_REGION -e GIT_COMMIT_SHA=$GIT_SHA $ECR_REGISTRY/$ECR_REPOSITORY:latest\"
  ]" \
  --region "$AWS_REGION" \
